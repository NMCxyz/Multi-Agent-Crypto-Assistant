...
from .doc_parser import CryptoFinancialDocParser
...
class CryptoFinancialRAG:
    """
    Crypto/Financial Retrieval-Augmented Generation system that integrates all components.
    """
    def __init__(self, config):
        ...
        self.logger.info("Initializing Crypto/Financial RAG system")
        self.config = config
        self.doc_parser = CryptoFinancialDocParser()
    ...
    def process_query(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None, session_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        print(f"🧠 1. DECOMPOSING QUERY: '{query}'")
        self.logger.info(f"🧠 1. DECOMPOSING QUERY: '{query}'")
        expansion_result = self.query_expander.expand_query(query)  # This returns a dict
        sub_queries = expansion_result["sub_queries"]  # Extract the list from the dict
        print(f"   📝 Original: '{query}'")
        print(f"   🔀 Sub-queries: {sub_queries}")
        self.logger.info(f"   📝 Original: '{query}'")
        self.logger.info(f"   🔀 Sub-queries: {sub_queries}")

        # Step 2: Multi-Query Retrieval
        print(f"🔍 2. MULTI-QUERY RETRIEVAL with {len(sub_queries)} sub-queries")
        self.logger.info(f"🔍 2. MULTI-QUERY RETRIEVAL with {len(sub_queries)} sub-queries")
        
        if len(sub_queries) > 1:
            print(f"   🔀 Running multi-query retrieval with {len(sub_queries)} sub-queries...")
            retrieved_documents = self.vector_store.retrieve_with_multiple_queries(
                queries=sub_queries,
                top_k_final=3  # Get top 3 chunks for better quality
            )
        else:
            # Use single query if only one sub-query (which is often the original query)
            print(f"   📝 Running single query retrieval...")
        ...
        if retrieved_documents:
            ...
            for i, doc in enumerate(retrieved_documents[:3], 1):  # Log first 3 chunks
                content = doc.get('content', '')
                print(f"      📄 Chunk {i}: Score={doc.get('score', 'N/A')}")
                print(f"         📂 Source: {doc.get('source', 'N/A')}")
                print(f"         📝 Full Content: {content}")
                print(f"         🔍 Content Length: {len(content)} chars")
                self.logger.info(f"      📄 Chunk {i}: Score={doc.get('score', 'N/A')}, Full Content='{content}'")
                self.logger.info(f"         📂 Source: {doc.get('source', 'N/A')}")
        else:
            print("   ⚠️ NO CHUNKS RETRIEVED!")
            self.logger.warning("   ⚠️ NO CHUNKS RETRIEVED!")

        # Step 3: Context Validation
        self.logger.info(f"🧪 3. VALIDATING CONTEXT SUFFICIENCY")
        validation_result = self.response_generator.validate_context_sufficiency(
            query=query,
            retrieved_docs=retrieved_documents
        )
        
        self.logger.info(f"   Context sufficient: {validation_result['sufficient']}")
        self.logger.info(f"   Reasoning: {validation_result['reasoning']}")
        
        if not validation_result['sufficient']:
            # Return early to trigger web search
            return {
        ...
        # Step 4: Rerank the retrieved documents if we have a reranker and enough documents
        self.logger.info(f"4. Reranking the retrieved documents")
        ...
        self.logger.info("5. Generating response...")
